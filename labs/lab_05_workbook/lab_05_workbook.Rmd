...
# It uses summarise() to create a single column, single row dataframe. 
  ...
  # In this newly created "cases_in_average_county" column, all of the county case values, which are stored in a column called "cases", are averaged together with mean() function to produce one number. 
...
# A: Vermont had the fewest total cases. The state had 1,608 cases.
```
Let's answer another, similar question. 
...
# A: Alaska had the fewest total COVID deaths, with 34.
...
# Write the answer in a comment
# Q: What was the average number of cases per county in each state? Which state had the lowest average?
# A: Vermont had the lowest average of cases per county. Overall, the average number of cases per county ranged from 114.85 in Vermont to 13,446.33 per county in Arizona.
...
# A: New Jersey had the highest average deaths per county with 758.90 average deaths per county.
...
# Write the answer in a comment
# Q: Which state has the most counties in our data? How many?
# A: Texas has the most counties in our data with 251 counties.
```
...
# A: D.C. only has one county, but Delaware is the state with the smallest number of counties. There are only three.
...
#Q: Which state had the lowest average deaths per county?
#A: Alaska had the lowest avergae deaths per county, with 1.307 average deaths per county.
