...
#The names of the columns for opioid involved overdose deaths and opioid prescriptions include a lot of extra numbers and punctuation. Also, starting with the 40th row, all of the following rows do not have a value for opioid involved overdose deaths. Instead, they have an asterisk (*) which supposedly means that the value did not meet inclusion criteria. Row 41 also has n/a for all columns. Lastly, the second two columns were read in as character columns, not number columns, so we can't compute with them.
